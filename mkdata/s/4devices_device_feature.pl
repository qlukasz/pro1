...
my $ddl_dmls = ();
my $create_chosen_table = "CREATE TABLE rent.devices_device_feature(
	id serial NOT NULL,
	id_device integer,
	id_device_feature integer,
	CONSTRAINT devices_device_feature_id PRIMARY KEY (id)
	
);";
push @$ddl_dmls, $create_chosen_table unless table_exists('devices_device_feature', 'rent', $dbh);
