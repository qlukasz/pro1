...
my $ddl_dmls = ();
my $create_chosen_table = "CREATE TABLE cwm.cwm_shifts_picked_date(
	id bigint NOT NULL DEFAULT nextval('cwm.cwm_shifts_picked_date_seq'::regclass),
	shift_id bigint,
	date date,
	CONSTRAINT cwm_shifts_picked_date_id PRIMARY KEY (id)
);";
push @$ddl_dmls, $create_chosen_table unless table_exists('cwm_shifts_picked_date', 'cwm', $dbh);
