...
my $ddl_dmls = ();
my $create_chosen_table = "CREATE TABLE cwm.cwm_shifts_picked_user(
	id bigint NOT NULL DEFAULT nextval('cwm.cwm_shifts_picked_user_seq'::regclass),
	shift_id bigint,
	user_id bigint,
	CONSTRAINT cwm_shifts_picked_user_id PRIMARY KEY (id)
);";
push @$ddl_dmls, $create_chosen_table unless table_exists('cwm_shifts_picked_user', 'cwm', $dbh);
