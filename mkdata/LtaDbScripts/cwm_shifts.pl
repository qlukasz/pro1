...
my $ddl_dmls = ();
my $create_chosen_table = "CREATE TABLE cwm.cwm_shifts(
	id bigint NOT NULL DEFAULT nextval('cwm.cwm_shifts_seq'::regclass),
	name character varying(100),
	from_hour time without time zone,
	to_hour time without time zone,
	date_add date,
	date_edit date,
	user_add bigint,
	user_edit bigint,
	task_type_id bigint,
	changed_at_least_one_event boolean DEFAULT false,
	CONSTRAINT cwm_shifts_id PRIMARY KEY (id)
	
);";
push @$ddl_dmls, $create_chosen_table unless table_exists('cwm_shifts', 'cwm', $dbh);
